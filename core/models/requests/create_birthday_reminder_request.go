...
type CreateBirthdayReminderRequest struct {
...
func (request *CreateBirthdayReminderRequest) Validate() (bool, int) {
	//check for required text fields
	if len(request.Image) == 0 || len(request.FirstName) == 0 || len(request.LastName) == 0 {
